...
def locations_in_warehouse(warehouse):
    weights, grid = read_warehouse(warehouse)
    worker = None
    boxes = set()
    goals = set()
...
class SokobanProblem(search.Problem):
    ...
    def result(self, state, action):
        ...
        if new_worker in boxes:
            new_box = (new_worker[0] + dx, new_worker[1] + dy)
            boxes.remove(new_worker)                   #removes location of worker from the boxes, then adds the new box lcoation to the list
            boxes.add(new_box)
